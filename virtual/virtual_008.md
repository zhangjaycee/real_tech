...
决定虚拟机是半虚拟还是全虚拟的性质转变的标准只有一个：Guest机知不知道自己是一个虚拟机。具体来说，不使用virtio，虚拟机会像在真实物理环境下运行一样地运行————它不认为它是虚拟机；而使用virtio，就是让存在于Guest和Host的两部分virtio程序的互相通信，这两部分程序分别称为前端驱动(frontend, Guest中)和后端设备(backend, Host中)，这样因为Guest中有了virtio的frontend部分，所以它的运行和物理机环境下有了区别，Guest按照一个使用virtio的虚拟机的方式运行————它知道了它是一个虚拟机。
